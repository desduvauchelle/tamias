...
export default function SkillsPage() {
	...
	const getChildren = (folder: string) => visibleSkills.filter(s => s.parent === folder).sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true }))
							...
							{topLevel.map(skill => {
								...
								return (
									<div key={skill.folder}>
										<button
											data-testid={`skill-item-${skill.folder}`}
										...
										{children.length > 0 && (
											<div className="ml-3 pl-3 border-l-2 border-base-300/60 space-y-0.5 mt-0.5 mb-1">
												{children.map((child, idx) => (
													<button
														key={child.folder}
														data-testid={`skill-item-${child.folder}`}
				...
				{isEditing ? (
					<div className="flex flex-col h-full p-6 p-6">
						...
						<div className="flex justify-end gap-2 pt-6 mt-4 border-t border-base-200/50">
							<button data-testid="skill-cancel-btn" onClick={() => setIsEditing(false)} className="btn btn-ghost">Cancel</button>
							{!activeSkill?.isBuiltIn && (
								<button data-testid="skill-save-btn" onClick={handleSave} className="btn btn-primary gap-2">
						...
						<div className="p-6 border-b border-base-200/50 flex justify-between items-start">
							<div>
								...
								<div className="flex items-center gap-2 mb-3">
									<div className="text-xs font-mono text-base-content/40 truncate max-w-lg" title={activeSkill.filePath}>
										{activeSkill.filePath}
									</div>
									{activeSkill.filePath && (
										<button
											onClick={() => {
												navigator.clipboard.writeText(activeSkill.filePath!)
							...
							<div className="flex gap-2">
								{!activeSkill.isBuiltIn && (
									<>
										<button data-testid="skill-edit-btn" onClick={() => openEditor(activeSkill)} className="btn btn-square btn-ghost btn-sm text-info hover:bg-info/10">
											<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
												<path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" />
											</svg>
										</button>
										<button data-testid="skill-delete-btn" onClick={() => handleDelete(activeSkill.folder!)} className="btn btn-square btn-ghost btn-sm text-error hover:bg-error/10">
											<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
					...
					<div className="flex-1 flex flex-col items-center justify-center p-8 overflow-y-auto">
						<div className="max-w-2xl w-full space-y-8 py-12">
							...
							<div className="flex flex-col items-center gap-4 pt-4 text-center">
								<button data-testid="skill-create-empty-btn" onClick={() => openEditor()} className="btn btn-primary btn-lg px-8 gap-3 shadow-lg shadow-primary/20 rounded-2xl">
