...
function Column({
	...
	const { data, isLoading, isError } = useQuery<DirData>({
		queryKey,
		queryFn: () =>
			fetch(`/api/files?path=${encodeURIComponent(fullQueryPath)}`).then(r => {
...
function FileModal({
	...
	const { data, isLoading, isError } = useQuery<FileContent>({
		queryKey,
		queryFn: () =>
			fetch(`/api/files/content?path=${encodeURIComponent(fullQueryPath)}`).then(r => {
				if (!r.ok) throw new Error('Failed to fetch')
				return r.json()
			}),
	})
			...
			{!hideHeader && (
				<div className="px-6 py-4 border-b border-base-300 shrink-0">
					<h1 className="text-lg font-bold font-mono text-base-content">
						File Navigator
						<span className="ml-3 text-xs font-normal text-base-content/30">{basePath ? `~/.tamias/${basePath}` : '~/.tamias'}</span>
					</h1>
