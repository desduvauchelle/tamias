 ...
 *
 * Use `createWorkspaceTools(sessionWorkspacePath)` to get per-session isolated tools
 * whose file operations are scoped to a specific directory (e.g. ~/.tamias/workspace/time-tracker).
 *
 * The bare `workspaceTools` export remains for backward compatibility and uses the global config path.
 */

export function createWorkspaceTools(sessionWorkspacePath?: string) {
	/** Resolve root from per-session path or fall back to global config */
	const getRoot = () => sessionWorkspacePath ?? getWorkspacePath()
	/** Validate a path within this session's workspace */
	...
	return {
		...
		search_grep: tool({
			...
			execute: async ({ pattern, path, include_extension, case_insensitive }) => {
				try {
					const root = getRoot()
					const targetPath = path ? vp(path) : root
...
/** Backward-compatible static export using the global config workspace path */
export const workspaceTools = createWorkspaceTools()
